...
class Googlenews
  ...
  def self.items(num = 10,topic = 'h', ned = 'us', q='')
    #puts "\n\n\n\n\nI am called\n\n\n\n"
    if(ned != '')
      options = { :query => { :num => num, :ned => ned, :topic => topic, :output => 'rss' } }
    else
      options = { :query => { :num => num, :topic => topic, :output => 'rss' } }
    end  
    features = self.get('/news', options)
    items = features['rss']['channel']['item']
    items.each do |item|
  ...
  # Get latest 10 or more articles
  def self.search_items(num = 10, q='')
    options = { :query => { :num => num, :q => q, :output => 'rss' } }
    features = self.get('/news', options)
    items = features['rss']['channel']['item']
    if !items.nil? 
      items.each do |item|
